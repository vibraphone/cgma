...
static const CubitString cgm_version = "9.0";
...
int cpp_cgm_initialize()
{
  ...
  VirtualGeometryEngine::instance()->register_attributes();
...
//   CubitAttribManager cam;
  
// //  CubitAttributeType att_type = CubitAttribManager::attrib_type("name");
//   CubitStatus result;
//   result = cam.register_attrib_type(CA_ENTITY_NAME, "name", "ENTITY_NAME", 
//                                                CAEntityName_creator, CUBIT_TRUE,
//                                                CUBIT_TRUE, CUBIT_TRUE, CUBIT_TRUE,
//                                                CUBIT_TRUE, CUBIT_FALSE);
//   assert (CUBIT_SUCCESS == result);
//   int att_type = cam.attrib_type("name");
...
//   cam.auto_actuate_flag(att_type);
//   cam.auto_update_flag(att_type);
                          ...
                          VertexHandle** vertexHdl )
{
  ...
  vertexPosition.print_me();
  
  RefVertex* refVertexPtr = NULL;

  //   // Construct a RefVertex
  refVertexPtr =
    GeometryModifyTool::instance()->make_RefVertex(vertexPosition);
  ...
  if ( NULL == refVertexPtr )
    {
     ...
     PRINT_INFO("Vertex creation succeeded\n");
                                  ...
                                  EdgeHandle** edgeHdl )
{
  ...
  for (int i = 0; i < numEdges; i++)
      ...
      if ( NULL == refEdgePtr )
	{
      ...
      refVertList.append(refEdgePtr->start_vertex());
      refVertList.append(refEdgePtr->end_vertex());
  ...
  MergeTool::instance()->merge_refvertices( refVertList, CUBIT_FALSE);
                             ...
                             FaceHandle** faceHandle)
{
  ...
  RefVertex* verts[3];
  verts[0] = GeometryModifyTool::instance()->make_RefVertex(centerPos);
  if(verts[0] != NULL)
    {
  ...
  verts[1] = GeometryModifyTool::instance()->make_RefVertex(max_x);
  if(verts[1] != NULL)
    {
  ...
  verts[2] = GeometryModifyTool::instance()->make_RefVertex(min_x);
  if(verts[2] != NULL)
    {
                                     ...
                                     double* distance)
{
  ...
  //Okay, we should have the closest_distance now, let's get signage
  CubitPointContainment is_in = ref_vol->get_body_ptr()->point_containment(location);
  switch(is_in)
    {
    case CUBIT_PNT_OUTSIDE:
      closest_distance *= -1.0;
                      ...
                      VolumeHandle** newVolumeHdl )
{
  ...
  // Perform the non-destructive boolean operation on the input Bodies.
  DLIList<Body*> tmp_list;
  DLIList<Body*> returned_list;
  tmp_list.append(bod1Ptr);
  tmp_list.append(bod2Ptr);
  CubitStatus status =
    GeometryModifyTool::instance()->unite( tmp_list, returned_list,
                                           true );
  ...
  if ( status == CUBIT_FAILURE )
  ...
  else
    {
      ...
      {
        PRINT_ERROR("Result of Unite was expected to be a single body.\n");
        return (-1);
      }
      
      newBodyPtr = returned_list.get();
      DLIList<RefVolume*> ref_vols;
