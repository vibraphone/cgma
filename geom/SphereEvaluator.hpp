//-------------------------------------------------------------------------
// Filename      : SphereEvaluator.hpp
//
// Purpose       : Provide evaluators for a spherical surface.  Since a sphere
...
#ifndef SPHERE_EVALUATOR_HPP
#define SPHERE_EVALUATOR_HPP
...
public:
    ...
    virtual GeometryType ask_type() const { return SPHERE_SURFACE_TYPE; }
};

class CUBIT_GEOM_EXPORT SphereEvaluator: public CubitEvaluator
{
...
public :
    ...
    SphereEvaluator( const SphereEvaluatorData *data ) { mEvalData = *data; }

  virtual ~SphereEvaluator()
    {}
  
    virtual GeometryType ask_type() { return SPHERE_SURFACE_TYPE; }
    ...
    virtual CubitBoolean is_periodic_in_U( double &period ) const { period = 2*CUBIT_PI;
                                                                    return CUBIT_TRUE; }
    virtual CubitBoolean is_periodic_in_V( double &period ) const { period = 2*CUBIT_PI;
                                                                    return CUBIT_FALSE; }
    virtual CubitBoolean is_singular_in_U() const { return CUBIT_FALSE; }
    virtual CubitBoolean is_singular_in_V() const { return CUBIT_TRUE; }
    ...
    //- Return the bounding obox of this sphere.
