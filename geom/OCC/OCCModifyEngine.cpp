                                       ...
                                       bool keep_old) const
{
  ...
  while( bodies.size() > 0)
  {
    ...
    if (revised_bodies.size() > 1)
    {
      ...
      stat =
        get_shape_list(revised_bodies, shape_list, is_volume, true); 
      ...
      B.MakeCompound(Co);
      for(int i = 0; i < shape_list.size(); i++)
        B.Add(Co, *shape_list.get_and_step());
                                            ...
                                            int gap_type )
{
  ...
  BRepOffsetAPI_MakeOffset offsetcurve(wire);
  offsetcurve.Perform(offset_distance);
  offsetcurve.Build();
  //For an open wire:
  //It returns a closed wire, with two connecting curves and a positive offset
  //curve list and a negative offset curve list.
  //For a closed wire:
  //It returns a closed wire outside of the original one if offset_distance > 0
  //It returns a closed wire inside of the original one if offset_distance < 0

  TopoDS_Shape wire_shape = offsetcurve.Shape();
  wire = TopoDS::Wire(wire_shape);

  BRepTools_WireExplorer Ex(wire);
  if(!closed)
  {
    //There should be 2*(size of curve list) + 2 curves, if more than that,
    //error out.
    int num_curves = 0;
    for(; Ex.More(); Ex.Next())
      num_curves ++;

    if((2+2*curves.size()) != num_curves)
    {
      PRINT_ERROR("Opencascade can't calculate one offset curve for each"
                  " input curve. It's a limitation, try use other type of"
                  " curves to offset.\n");
      return CUBIT_FAILURE;
    }
